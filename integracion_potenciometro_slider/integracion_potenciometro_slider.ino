...
int getAngleFromPosition(int pos) {
  return map(pos, 0, maxPosition4, 90, 0); // 0 pasos = 90°, 100 pasos = 0°
}
...
void moveMotor4To(int targetPos) {
  if (targetPos == currentPosition4) return;

  direction4 = (targetPos > currentPosition4) ? 1 : 0;
  int steps = abs(targetPos - currentPosition4);
...
void buscarPosicionValidaAlInicio() {
  ...
  if (pot >= POT_MIN && pot <= POT_MAX) {
    Serial.println(" - Potenciómetro ya en rango, no se necesita mover el motor.");
    return;
  }

  Serial.println(" - Pot fuera de rango, buscando posición válida...");

  const int maxIntentos = 150;
  bool direccionSubir = (pot < POT_MIN); // true = subir (horario)

  digitalWrite(DIR4, direccionSubir ? LOW : HIGH); // HIGH = subirHIGH
  ...
  for (int i = 0; i < maxIntentos; i++) {
    ...
    pot = getPotReading();
    if (pot >= POT_MIN && pot <= POT_MAX) {
      Serial.println(" - Rango válido alcanzado.");
      break;
    }

    // Seguridad: parar si llega al límite físico
    if (currentPosition4 == 0 || currentPosition4 == maxPosition4) {
      Serial.println(" - Límite físico alcanzado sin encontrar rango válido.");
      break;
    }
  }
...
<h2>Lectura Potenciómetro</h2>
<div>Valor: <span id="potValue">---</span></div>
...
void setup() {
  ...
  server.on("/startRight1", []() { motorActive1 = true; movingRight1 = true; digitalWrite(DIR1, LOW); server.send(200, "text/plain", "Right"); });
  server.on("/startLeft1",  []() { motorActive1 = true; movingRight1 = false; digitalWrite(DIR1, HIGH); server.send(200, "text/plain", "Left"); });
  server.on("/stop1",       []() { motorActive1 = false; server.send(200, "text/plain", "Stopped"); });
  server.on("/start2",      []() { motorActive2 = true;  server.send(200, "text/html", html); });
  server.on("/stop2",       []() { motorActive2 = false; server.send(200, "text/html", html); });
  server.on("/start3",      []() { motorActive3 = true;  server.send(200, "text/html", html); });
  server.on("/stop3",       []() { motorActive3 = false; server.send(200, "text/html", html); });
  ...
  server.on("/pot", []() {
    int pot = getPotReading();
    server.send(200, "text/plain", String(pot));
  });
  ...
  motorPulseTicker3.attach_us(30, togglePulse3);
}
